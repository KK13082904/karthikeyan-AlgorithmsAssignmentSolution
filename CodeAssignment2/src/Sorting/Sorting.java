...
public class Sorting {
	...
	void ascMerge(double nums[], int left, int m, int right) {
	    int n1 = m - left + 1;
	    int n2 = right - m;
	...
	public void Sort(double[] nums, int left, int right,int type) {
		...
		if (left < right) {
	        ...
	        if(type==1) {
	        	ascMerge(nums, left, m, right);
	        }else if(type==2) {
	        	descMerge(nums, left, m, right);
	        }
	    }
		
	}
	
	public static void descMerge(double[] arr, int left, int mid, int right) {
		int len1 = mid - left + 1;
		int len2 = right - mid;
